...
channel_alias = {'C3': ['C3-A2', 'EEG C3-A2', 'C3-M2', 'C3-avg', 'C3-x'],
                 'C4': ['C4-A1', 'EEG C4-A1', 'C4-M1', 'C4-avg', 'C4-x'],
                 'Chin': ['Chin1-Chin2', 'Chin EMG'],
                 'Leg': ['LLEG1-RLEG1','LAT-RAT'],
                 'Arm': ['ARM1-ARM2'],
                 'EOGL': ['EOGL','LEOG-M2','EEG LOC-A2','LEOG-M2','LEOG-x'],
                 'EOGR': ['EOGR', 'EEG ROC-A1','REOG-M1','REOGM1','ROCM1','ROC-M1','ROCA1','ROCA2','REOGx','REOG-x'],
                 'ECG': ['ECG','EKG1-EKG2', 'ECG ECGL-ECGR','EKG1AVG','EKG1EKG2','LLEG1EKG2','EKG'],
                 'Airflow': ['Airflow','AIRFLOW','OralTherm','NasaslTherm','Nasal/OralTherm','Resp FLOW','FLOW'],
                 'NasalP': ['NasalPres','NasalP','NASAL PRES','Cannula Flow'],
                 'Abd': ['ABD', 'ABDO EFFORT', 'Abd','Abdomen','Abdominal','Resp ABD'],
                 'Chest': ['CHEST', 'Chest', 'Resp CHEST', 'THOR EFFORT','Thoracic'],
                 'OSat': ['OSAT','SpO2','SaO2'],
                 'Mic': ['Snore','SNORE']}
...
def plot_edf_data(data, channels):
    x = data['x']
    fs = data['fs']
    n = len(x)
    fig, axs = plt.subplots(n, 1, figsize = (10,10), dpi = 300)
    for i in range(n):
        N = len(x[i])
